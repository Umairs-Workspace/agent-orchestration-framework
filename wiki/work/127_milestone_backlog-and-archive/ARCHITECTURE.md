...
| FF-12705 | **Archive never renumbers.** `src/commands/archive.mjs` imports neither `src/work/reindex.mjs` nor `src/commands/insert-shared.mjs` (transitively, over the graph's dependencies of the file) and its source contains no `number:` write and no `parseInt`; its only frontmatter-touching call is none — the link rewriter is asserted to match only relative-path link lines (`](../` / `](./`), never a `^number:` line. Red probe: import `reindex.mjs` into `archive.mjs`; and separately write `number:` in the link rewriter. | `test/arch/work/acd-archive-never-renumbers.test.mjs` *(pending — 127/03)* | ADR-004 |
