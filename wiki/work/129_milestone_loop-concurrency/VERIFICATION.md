...
- **The fitness tier, `node scripts/test-rubric.mjs` — what `work:grade` runs — under an isolated
  ...
  share of `F-09` is cleared; the rejected `lanes.mjs` under `src/loop/` remains the architect's.
...
| F-58 | A provider wait was read as silence: when the account's usage limit is hit, `claude` prints `Usage limit reached · continuing automatically at <time>` / `You've hit your session limit · resets <time>` and waits, alive, for the reset; no transcript progress follows, so the driver's heartbeat deadline killed the session as `timeout` — attempt 1 of 127/03's refine at minute 40 (after authoring the whole contract inline, its own agents having died on the same 429), attempts 2–4 after 20 minutes each with 5 seconds of activity — an hour of blind retries against a limit no retry lifts (loop 127, 2026-09-15 10:47–12:27Z, `loop-diag.127.2026-09-15T10-47-32-544Z.log`, transcripts `7fe3b9fa`, `af04c236`, `df976462`, `5a05001b`). | defect | medium | fixed at 06's build (task 01, `@bug @finding-F-58`): `src/agent-session-driver.mjs` reads the line off the output tail (escapes stripped, `PROVIDER_WAIT_RE`, both spellings); while the last such line is newer than every heartbeat the check re-asks a window later and kills nothing; the first heartbeat after it restores the rule; `startToCloseMs` still bounds the attempt; reported once as the `provider-wait` breadcrumb. Two rows + the pattern's five rows in `four-deadlines`; red with the guard reverted. | `129/06` | closed |
| F-59 | The liveness probe races the stop the driver itself requested: `stopForOutcome({ done })` records the outcome, kills the tree, and applies it only when `term.onExit` delivers; the 15-second `process.kill(pid, 0)` probe saw the killed pid first and settled `failed/agent_died` (`finish` is settled-guarded, so the `done` was a no-op). Attempt 5 of 127/03's refine completed, ran `run-complete done`, the driver logged `stop-requested done` → `tree-terminated` → `exit-confirmed failed` 55 ms apart, and the loop halted `run-not-retryable` on a refine that had finished (record `20260915T122747029Z-0004`, transcript `bd607551`). The same kill site as the 2026-09-12 loop deaths, one layer down: the loop survived, the record lied. | defect | blocker (for the live run) | fixed at 06's build (task 01, `@bug @finding-F-59`): the probe settles `requestedStopOutcome ?? agent_died` — the requested outcome is the truth it honours, a death nobody asked for is still `agent_died`. Two rows in `mesh-worker-liveness`; red with the fix reverted. | `129/06` | closed |
| F-60 | The read-only probe (`aof work loop <NN> --json`, `probeLoop`) decides without the mode: on 127 it answered `act: gate 127/02` while the foreground loop's first act under `refine_first` was `drive refine 127/03 --orchestrated` — `nextDecision` is asked without `concurrency` / `unrefined`, so the operator's pre-run read disagrees with the run. | reporting-gap | low | non-blocker: the probe resolves the mode and gathers `unrefined` as the body does (`src/commands/loop.mjs`, one call away); not taken at the live run. | `src/commands/loop.mjs` (future item) | open |
...
| F-69 | `aof work regression-gate 129` at `a8e0e14` (this door's first row, 2026-09-22T00:47Z, 59 min on a clean detached worktree): six reds, two of them real — `command-core/00 … work:loop run is async` (`src/commands/loop.mjs`'s `run` had become a plain arrow returning a promise) and `grade/01 … loop/loop-command-stops names grade-indeterminate nowhere` (the stops suite had gained a copy of the frozen fifteen-member `LOOP_STOPS` literal that `test/arch/loop/acd-loop-probe-contract` already pins). Both arrived with 130/02's lane commit `48ed32b` (2026-09-21) and were green at 127's gate (`f27d7d6`). Run 2 at the repair (`16850c7`, 09:27Z, 38 min) then redded 130's own `FF-13003` (`acd-loop-stop-request-single-home`), which pins the dispatch line's exact text WITHOUT the `async` keyword `command-core/00` requires — two of 130's controls contradicting each other over one line. | defect | medium | repaired at the owner at this door: `run` is `async` again and the stops suite asserts the vocabulary's length and its twelfth member, citing the standing pin (`16850c7`); FF-13003's regex admits `async` (`6a81d14`). Behaviour unchanged; blast radius three files, +11 / −3; every touched suite green alone in the worktree (83 / 0, then 57 / 0). Recorded for 130's retrospective in its `STATE.md` feedback. | `130/02` (repaired here) | closed |
| F-70 | Four mesh cases red under gate runs 1–3 and green alone in the same worktree at the same commit: `task05/38 clone-credential-pull` ×3 and `task01/38-07 push-before-worktree-removed`. First labelled the contention species `124/F-17` / `127/F-24` name; that label was wrong — see `F-71` for the measured cause (the launcher's environment, not load). | test-gap | low | closed by `F-71`'s measurement: not contention, not a flake — deterministic under the launching shell's environment. The row is kept so the label's correction is on the record. | `129` | closed |
| F-71 | The four mesh cases of `F-70` are red on EVERY whole-tree run launched from the PowerShell tool session (three of three: `a8e0e14`, `16850c7`, `6a81d14`) and green under every run launched from the Bash tool session (alone, 51 / 0; the whole `test/mesh/` lane, 135 suites, 1,074 / 0). One raw `scripts/test.mjs` run at `6a81d14` with stderr retained (7,363 ok, 4 not ok) names the assertion at all four sites: `'' !== undefined` — `no GIT_ASKPASS on a public/no-resolver clone` / `… for a legitimately-declined credential` / `no askpass pointer when there is no credential to carry` / `GIT_ASKPASS never lands on ambient process.env`. The launching PowerShell session carries `GIT_ASKPASS=""` (present, empty) and `GIT_TERMINAL_PROMPT=0`; the Bash session carries neither; the runner's per-test isolation covers `AOF_GLOBAL_HOME` only, so a launch-time variable is the test process's for the whole run. Nothing in `src/` or `test/` writes it (no `process.env.GIT_ASKPASS` writer exists; none was added since the green 127 gate). `aof work regression-gate` retains case names only, which is why three rows could not say this. | environment | medium | not a repo defect and not a blocker for `129/06`; it blocked the milestone door only while unnamed. Repair is the launch: `Remove-Item Env:GIT_ASKPASS; Remove-Item Env:GIT_TERMINAL_PROMPT` before `Start-Process`, or launch from Bash — recorded in the gate route. Two follow-ons for their owners: the four cases could assert `!process.env.GIT_ASKPASS` and stay honest (milestone 38's lane); the gate's row could carry each failure's first assertion line, not only its name (`src/commands/regression-gate.mjs`, ADR-008's record). | operator (launch env); 38 mesh lane + `regression-gate` (future items) | open |
| F-72 | Every whole-tree run at this door CRASHED before its integration and cargo lanes: `test/store/global-work-propagation.test.mjs` case /03 starts the real launcher as the control node, which starts the real control stream server on the fleet port (`mesh.relay.url` unset → `DEFAULT_CONTROL_SERVICE_PORT` 4182), and on this control node the live fleet daemon holds it — the raw run's stderr ends in an unhandled `listen EADDRINUSE 127.0.0.1:4182` after ~7,360 unit cases, and the gate's row reads "the runner exited 1 and enumerated no failure" (run 4) or lists only the not-oks that preceded the crash (runs 1–3). `.claude/rules/build-deploy-restart.md` names exactly this binder; 127's green gate ran while the daemon was down. | defect | high | **blocker for the milestone door** until a whole-tree run completes. Fixed at the owner's fixture at this door: case /03 injects the launcher's `startControlStreamServer` seam with the `{ stop, updatePeers }` fake five launcher suites already use — the case measures the propagation ticker, not the listener; blast radius one file, +5 / −0, behaviour unchanged. The rule file's "never run the full suite here" sentence is now stale for the gate route and is the operator's to amend. | `test/store` (m42, repaired here) | closed |
| F-73 | With the binder re-homed, gate run 5 (`f820ae9`, 12:44Z, 58 min — the first run at this door to reach its end) redded 127/05's link ratchet: `3067 >= 3069` — two links fewer than `LINKS_BEFORE` pinned at `ba25547`. The two are the `src/mesh-ui-serve.mjs` citations inside a `TECH_DEBT.md` entry that 130/03 DISCHARGED (`c55b2f1`, 2026-09-21) — a shrink-only ledger deletes paid entries by design, and a whole-tree floor on the link COUNT reds on every such discharge forever; nothing was lost by the archive move the ratchet guards (resolving ROSE 2312 → 2317, the into-archive legs untouched). | defect | low | repaired at the owner (`5d71f01`): `LINKS_BEFORE` re-measured at `f820ae9` (`total: 3067, resolving: 2317`) with the reason on the pin. The floor stays a floor; the next discharge will move it again, which is the control's own design to revisit (127/05's, a future item). | `127/05` (repaired here) | closed |
| F-74 | Two more reds of the same run, both from the backlog story captured at 127's accept (`4318bcc`, `story_run-records-carry-the-node-id`, the F-12 follow-on): (1) the whole-stream validate flagged its untouched scaffold (`reads: []` + `files: []` is the template's own signature, by the rule's text), so 127/05's "validate and doctor from the repository root are no redder" leg and its whole-tree validate leg were red — `aof:add-story` scaffolds exactly the shape validate refuses, so EVERY captured backlog story reds the tree until it is refined; (2) 127/05's "an item added under intake backlog" case assumed an EMPTY backlog — a pre-existing backlog row sorting before the added one has `number: null`, and the case asserted `undefined` for every row before it. | defect | medium | repaired at the owners (`5d71f01`): the stub's two empty scaffold lines dropped (absence is the rule's own valid state — "221 stories predate the field" — and `continue`'s missing-reads stop still refuses to drive it), the whole-stream validate PASS again; the ordering case admits backlog rows before the added item, its claim (no live row after it) unchanged. The scaffold-versus-validate disagreement itself is a product item for the operator: either `add-story` into the backlog scaffolds without the empty pair, or validate exempts an unnumbered item. | `127/05` + `add-story` scaffold (operator, future item) | open |
| F-75 | The last red of run 5, and the one this door cannot clear: 127/05's "the root of wiki/work holds only live items" — `127 (127_milestone_backlog-and-archive) at the root is not done (status: done)`. 127 was accepted 2026-09-17 (`9e6623a`) and never archived; 127/ADR-004 makes `aof work archive <NN>` the operator's act, never the accepting ceremony's, while 127/05's own control reads a `done` driver at the root as a broken tree — so every gate after an accept is red until the operator archives. Re-run alone at `5d71f01` with the other four repaired: 18 / 1, this case the one. | process | medium | **blocker for the milestone door** until `aof work archive 127` runs (the operator's act), then a sixth gate run; not a blocker for `129/06`. The rule conflict (a separate archive act versus a root that may never hold `done`) is the operator's to resolve — archive at accept, or let the control admit `done` at the root. | operator (`aof work archive 127`; the rule) | open |
| F-76 | `aof work archive 127` (`b0748bf`, the operator's act, 2026-09-22) cleared `F-75` and redded two more of 127/05's cases over the tree it produced: the board-face case named 127 as THE live milestone with stories (`byDefault.items.some(row => row.parent === "127")`), and the into-archive link leg counted 51 broken links against a ceiling of 48 — the three are link-SYNTAX examples in 127's own docs (`[…](target)` in 127/03's `OUTCOME.md` and `PLAN.md`, a bare backtick target in 127's `ARCHITECTURE.md`) that the census reads as links, broken before the move inside the root folder and now "into archive/" because they resolve there. The verb rewrote 38 crossing links in one file and nothing else needed rewriting. | test-shape | low | repaired at the owner (`bec9c0f`): the board-face case reads a live milestone with stories off the live roots (non-vacuous), and `BROKEN_INTO_MOVED_BEFORE` is re-pinned 48 → 51 with the reason on the pin. The suite alone: 19 / 0. Both assumptions were true on the day 127/05 shipped and false the day its own milestone was archived — the suite's whole-tree legs are pinned to a tree that every archive changes, which is 127/05's design to revisit (a future item). | `127/05` (repaired here) | closed |
| F-77 | Gate run 6 at `bec9c0f` (17:48Z, 58 min — the first run with 127 archived) was red on ONE case, 53/00 task03's "any movement anywhere in the session tree restarts the quiet stretch". Not load: run it alone and it reds about one run in three (measured 1 of 3 here, `true !== false`). The case awaits an mtime bump and then advances a virtual clock; the await yields to the event loop, so the watch's 10 ms poll can observe the movement at the OLD clock instant, and the subsequent advance then settles the watch the assertion expects to be unsettled. | test-gap | medium | repaired at the owner (`deea144`): a synchronous `bumpMtimeSync` twin bumps the file in the SAME turn as `clock.advance`, so no poll runs between them; the assertion, the window and the async helper's other eleven call sites are untouched. Five consecutive runs of the suite alone: 22 / 0 each. | `53/00` (repaired here) | closed |
...
done` — waits on that ruling. `aof work regression-gate 129` ran at this door three times on a
clean detached worktree (119/R4's route); the rows are in `REGRESSION.md` and the reading is the
paragraph below — the newest row is red on four mesh cases whose cause is `F-71`.

**The gate at this door (2026-09-22) — five runs, `REGRESSION.md`, the newest RED on one case
that is the operator's to clear.** Run 1 at `a8e0e14` (00:47Z, 59 min): two real reds, both
130/02's (`F-69`), plus four mesh cases. Run 2 at the repair `16850c7` (09:27Z, 38 min): 130's own
`FF-13003` over the repair's `async` keyword (`F-69`), plus the same four. Run 3 at `6a81d14`
(10:12Z, 43 min): the four alone. Their cause, read off one raw whole-suite run with stderr
retained: the launching PowerShell session's `GIT_ASKPASS=""` (`F-71`), and — behind it — every
run so far had DIED on `EADDRINUSE :4182` before its integration and cargo lanes, which the gate's
name-only row could not say (`F-72`). Run 4 at `6a81d14` with a clean launch environment (11:42Z,
40 min): "the runner exited 1 and enumerated no failure" — the crash, now visible. Run 5 at
`f820ae9` with the binder re-homed (12:44Z, 58 min): the first run to reach its end — five reds,
all in 127/05's tree-shape suite and all consequences of 127's accept: the link ratchet (`F-73`),
the backlog stub's scaffold signature and the empty-backlog ordering assumption (`F-74`), and 127
itself `done` at the root (`F-75`). Four of the five are repaired at their owners (`5d71f01`; the
suite alone: 18 / 1); the fifth is `aof work archive 127`, the operator's act by 127/ADR-004. What
is NOT true: that the environment cannot host the run — with the binder re-homed it runs to a
