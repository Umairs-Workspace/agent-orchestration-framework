...
| F-70 | Four mesh cases red under gate runs 1 and 2 and green alone in the same worktree at the same commit (51 / 0 with the real reds excluded): `task05/38 clone-credential-pull` ×3 and `task01/38-07 push-before-worktree-removed` — the spawn-heavy contention species `124/F-17` and `127/F-24` name; run 1 took 59 min and run 2 38 min, so the contention is the full run's, not the machine's load. | test-gap | low | non-blocker; recorded so the door's reader knows which reds are contention. The newest row is the one the door reads. | `129` | closed |
| F-71 | The four mesh cases of `F-70` are red on EVERY whole-tree run at this door (three of three: `a8e0e14`, `16850c7`, `6a81d14`) and green alone and with their whole lane (`test/mesh/`, 135 suites, 1,074 / 0) — consistent, not random, so "contention" is a label and not yet a cause. `aof work regression-gate` records case NAMES only, so the assertion that fails under the full run was never seen by anyone. | test-gap | medium | **blocker for the milestone door** while it stands (the newest gate row is red and `done` is refused by `regression-gate-red`); not a blocker for `129/06`. Under measurement: one raw `scripts/test.mjs` run in the gate worktree at `6a81d14` with its TAP retained, launched 2026-09-22 10:18Z; its `not ok` messages name the cause and its owner (the four suites are milestone 38's; the interaction is cross-lane, since the mesh lane alone is green). Then: repair at the owner and a fourth gate run. | `129` door (measure); the four suites' owner (repair) | open |
...
**The gate at this door (2026-09-22) — three runs, `REGRESSION.md`, the newest RED.** Run 1 at
`a8e0e14` (00:47Z, 59 min): six reds — two real, both 130/02's (`F-69`), four the mesh contention
species (`F-70`). Run 2 at the repair `16850c7` (09:27Z, 38 min): 130's own `FF-13003` red over the
repair's `async` keyword (`F-69`, the control's expectation widened) plus the same four. Run 3 at
`6a81d14` (10:12Z, 43 min): the same four and nothing else — `task05/38 clone-credential-pull` ×3 and
`task01/38-07 push-before-worktree-removed`, green alone (51 / 0), green with their whole `test/mesh/`
lane run together (135 suites, 1,074 / 0), red only under the whole-tree run, three times out of
three. The gate retains names, not messages, so the cause is being read off one raw whole-suite run
with its TAP kept (`F-71`); until it is named, the newest row is red and `aof work status 129 done`
is refused by `regression-gate-red` — the door's own design, and the right answer while a red has no
owner. What is NOT true: that the environment cannot host the run (it ran to a verdict three times
with the fleet daemons up), so `--gate-override` is not the escape here; the escape is a named cause
and a fourth green row, or the four cases' owner (milestone 38's mesh clone/push lane) making them
hold under load.
