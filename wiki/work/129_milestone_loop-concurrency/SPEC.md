...
- [x] `06_story_the-second-live-run` — `refine_first` switched on here and one real `aof work loop` driven over a two-member wave with a held third, every SPEC outcome read at the source (`@manual`)
