...
(`src/commands/loop.mjs:1583`, `:1833`), which is an hour or more into a story session. The
exit-reason recorder (`src/loop-diag.mjs`) registered a persistent listener per signal that only
wrote a line — and any listener disables node's default exit — so once the shell's `once` was
consumed every further Ctrl+C was logged and ignored: eleven `signal SIGINT` lines at 11:41Z in
`loop-diag.129.2026-09-13T08-26-34-907Z.log`, pid 82056 alive until the driver's own deadline
ended the session at 11:45Z. That listener is repaired at `9f6be4be` (a signal with no other
listener restores node's default, exit `128 + signo`) and is not this milestone's work. What the
repair reveals is the shape of the gap: the second Ctrl+C now kills the process outright — the
loop's own halt path is still "wait for the drive" — and on Windows nothing outside the loop's
console can send that signal at all. The fleet (`http://127.0.0.1:4181/?mode=fleet`), the board
and the desktop app have no "stop this loop" affordance; the desktop owns only `--supervised`
loops, and its Stop is a bare `child.start_kill()` of the direct child.

The same log records a second defect the stop must close. When the 129 loop's in-flight drive
finally ended on the driver's `timeout` (`stop-requested … failureReason: "timeout"`, tree
terminated, exit confirmed), the interrupt branch returned BEFORE `settleDriven`
(`src/commands/loop.mjs:1833-1838`): 129/04's run
(`stories/04_story_the-wave-tick/runs/umamis-msi/20260913T110303238Z-0000.json`) is `running` to
this hour with the driver's observation discarded — a record only a stale-run reclaim can close.
A stop that leaves the run it stopped in flight is not a stop.

The mesh already has the precedent for a control-side stop of an agent session: assignment
WITHDRAW (`aof mesh assign <ref> --withdraw`) kills the live session through the driver's
`onPtyLive(kill)` seam and settles its run `cancelled` (`src/mesh/worker-execution.mjs:1476-1542`);
`running>cancelled` is a legal edge and a clean cancel records `failureReason: null`, which
`retryReadiness` reads as `not-retryable` (`src/run-store.mjs:271-277`, `:437-447`). 129/02 gave
the driver an `options.signal` (an `AbortSignal`) that requests `{ failed, cancelled }` through the
SAME stop bracket every other stop takes — stop-requested, tree-terminated, pty-released,
exit-confirmed (`src/agent-session-driver.mjs:1253-1269`) — and the in-process drive already
spreads `ctx.agentSessionDriverOptions` into the driver (`src/commands/drive.mjs:263`, `:289`), so
a signal placed there reaches a live session with no new seam. 129/04's task 06 fixes the
interrupt semantics at the wave grain — the first signal drains, the second cancels every child
through stdin's end — against an INJECTED signal seam. The stop request this milestone adds IS that
seam's producer: a second one is not invented.
