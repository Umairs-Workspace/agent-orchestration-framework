...
Feature: the live stop, read at the source — the verb, the fleet and the desktop each stop a real loop on this machine, the record settles cancelled, nothing relaunches it, and --resume brings it back
  ...
  Background:
    ...
    And `<L>` is `brief.loop.loopRunId` of the newest `runs/umamis-msi/*.json` under the driven item, and `<path>` is `~/.aof/mesh/loop-stops/<L>.json`
  ...
  Scenario: PRECONDITION — the payload is installed and the OPERATOR restarted the desktop
    ...
    Then the newest `daemon-started` entry in `~/.aof/mesh/logs/mesh-serve.log` reads `mesh serve running (node umamis-msi, build payload <buildId>)` and in `mesh-ui.log` reads `mesh ui running (build payload <buildId>)`, both with `at` after the relaunch, both pasted
  ...
  Scenario Outline: a failure signature is a finding against the owning story, never a re-try
    ...
    Examples:
      ...
      | a new `runs/umamis-msi/*.json` for `<ref>` within the two ticks after a honoured stop — the desktop relaunched it | 04      |
  ...
  Scenario: leg 1 — the verb drains, the run settles as it ended, and resume clears
    ...
    Then T2 prints `<scope> — stop requested (drain) for loop <L>, live. <path>` and exits 0, and `<path>` (pasted whole) reads `level: 1`, `state: "requested"`, `escalatedAt: null`, `honouredAt: null`, `cancelled: null`, `by: { node: "umamis-msi", pid: <T2's aof pid> }`
    And T1 finishes the in-flight drive and `<log>` shows `stdout Driven <ref> — <phase> (<outcome>).` then `stdout <scope> — halted on operator-interrupt at <ref> (producer stop-request). Resume with: aof work loop <scope> --resume Details: signal=stop-request; level=1; request=<path>; by=umamis-msi:<pid>.` then `exit code=0` — the three lines pasted with their instants
    And the drive's record (the newest `runs/umamis-msi/*.json` for `<ref>`, pasted whole) reads `state` `done` or `failed` as the drive ended — never `running` — and `<path>` now reads `state: "honoured"`, `honouredAt` set, `cancelled: null`
  ...
  Scenario: leg 3 — a stop in the between-drives gap answers not live and still stops the next tick
    ...
    And within 2 s of the next tick head T1 prints no further `Driving …` and `<log>` shows `stdout <scope> — halted on operator-interrupt at <ref or scope> (producer stop-request). … Details: signal=stop-request; level=1; request=<path>; by=umamis-msi:<pid>.` then `exit code=0` — no drive started, no new record under `runs/umamis-msi/`
  ...
  Scenario: leg 4 — the desktop does not relaunch a stopped loop, and shows the row's end
    ...
    And `Get-ChildItem runs/umamis-msi/` for `<ref>` (pasted) lists no file newer than the cancel's `<runId>` — nothing relaunched it
    When `aof work loop <scope> --resume` runs in T1
  ...
  Scenario: leg 5 — the fleet's button walks the two rungs
    ...
    Then the body's `localNodeId` is `"umamis-msi"` and the `umamis-msi` node's `presence.loops[0]` is the eleven-key entry with `loopRunId: "<L>"` and `stop: null`; the card's current-work region shows `loop <scope> · <phase> <ref> · cycle <n> of <cap>` with a `Stop` button; `umamis-mac-mini`'s card shows any loop line of its own with no button — the body pasted, the cards described
    ...
    Then the line reads `· cancelling ·` with no button; `<log>` shows leg 2's bracket and halt with `level=2` and `cancelled=<runId>`; `runs/umamis-msi/<runId>.json` reads `"state": "cancelled"`; within 20 s the line is gone, the body's `presence` carries no `loops`, and `running N runs` decrements — the record and the body pasted
  ...
  Scenario: leg 6 — the desktop's row walks the ladder without the fallback
    ...
    Then `<path>` reads `level: 2`; `<log>` shows the bracket, `stdout Driven <ref> — <phase> (cancelled).`, the halt with `level=2; … cancelled=<runId>` and `exit code=0`, that instant inside 30 s of the second press; `runs/umamis-msi/<runId>.json` reads `"state": "cancelled"` — no `taskkill` reached the loop
  ...
  Scenario: a remote loop is refused by name
    ...
    Then it exits non-zero with code `loop-stop-not-local`, the message naming `umamis-mac-mini`, `umamis-msi` and "stop it on umamis-mac-mini's own console", and `Get-ChildItem ~/.aof/mesh/loop-stops/` gains no file — stderr and the listing pasted
