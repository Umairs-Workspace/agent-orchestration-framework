...
| F-16 | Gate repair, 137's: the session driver's static reach went 24 → 25 through `src/work.mjs`'s static import of `digest-template.mjs` (FF-5301). | defect | blocker (gate) | inherited — repair at owner | 137 (FF-5301's ceiling) | fixed at 130's door. A deferred `import()` was tried first and went red on FF-5407 at gate run 1 (`work.mjs` may not dynamic-import). The operator then ruled the ceiling raised 24 → 25, stated in the control: a pure leaf whose one import the driver already reaches. |
...
| F-19 | Gate repair: FF-11903 read 61 unresolved citations against 54. Five are in 131's uncommitted docs, which a clean gate does not read. One was 130's own VERIFICATION (a probe's misspelt needle written as a path), now respelt. Measured clean in a detached worktree: 56. 130's two modules cleared, three are 134's refine citing modules its build will land, and one is 134's SPEC citing an `observe.mjs` at the `src/` root, a module that never existed. | defect | blocker (gate) | repair + re-pin at the door, argued in the row | 134 (`SPEC.md`), 130 (FF-11903's ceiling note) | fixed: 134's SPEC re-pointed to `src/work/observe.mjs`; the ceiling re-pinned 54 → 55 with the argument in the row, due to fall to 52 at 134's accept |
| F-20 | Gate run 2 (at `d5f5d44`, first to pass the arch lane), 137's residues: `verification-template` ×2 does not name 137's `AOF.md` in `templates/milestone/`, and `archive-is-a-move 00` pins `src/work` at 44 (the F-15 raise). | defect | blocker (gate) | inherited — repair at owner | 137 (two suites) | fixed at 130's door: `AOF.md` named in the shipped-before list; the pin moves to 45 with its reason |
| F-21 | Gate run 2: done drivers still at the root of `wiki/work` (`129`, `132`, `137`). `work-this-tree-holds-what-is-live 02` refuses that. | process | blocker (gate) | operator's act (127/ADR-004) | the operator: `aof work archive` | open |
