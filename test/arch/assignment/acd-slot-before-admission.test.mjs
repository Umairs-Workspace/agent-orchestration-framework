...
export function productionAdmissionPathProblems(listing) {
  const problems = [];
  let localCalls = 0;
  ...
  for (const file of listing ?? []) {
    ...
    if (opens > 0) {
      localCalls += opens;
      if (rel !== "src/commands/dispatch.mjs") problems.push(`${rel} opens a local dispatch lane outside the bounded production door`);
    }
  ...
  if (localCalls !== 1) problems.push(`expected exactly one production resolveDispatchLane call, found ${localCalls}`);
