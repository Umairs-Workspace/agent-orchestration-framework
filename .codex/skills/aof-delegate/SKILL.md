...
Use this skill when the user asks for `$aof-delegate on | off | status`, or asks to run the AOF `aof:delegate` procedure in Codex.

Where this procedure mentions `$ARGUMENTS`, use the text the user supplied after the skill name.
Where it mentions Claude slash command `/aof:delegate`, treat that as this Codex skill invocation.
